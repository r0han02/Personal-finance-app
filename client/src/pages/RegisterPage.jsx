  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="subtitle" style={{ textAlign: 'center' }}>Start managing your finances with FinanceFlow</p>
        {error && (
        ...
        <form onSubmit={handleSubmit}>
          <motion.div className="form-group" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            <label>Name</label>
            <input className="form-control" type="text" placeholder="John Doe" value={form.name} onChange={set('name')} />
          </motion.div>
          ...
          <motion.div className="form-group" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
          ...
          >
            <span data-scramble>{loading ? 'Creating account…' : 'CREATE ACCOUNT'}</span>
          </motion.button>
        </form>
        <p className="auth-footer">
          ...
          >
            <span data-scramble>Sign in</span>
          </Link>
        </p>
      </motion.div>
    </motion.div>
